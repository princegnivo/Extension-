...
(function () {
  ...
  // 1) Injecte le hook Socket.IO — dans CHAQUE frame (au cas où le
  // graphique/flux serait dans une iframe plutôt que la page principale)
  function injectHook() {
  ...
  // 2) Réception des messages du script injecté (même frame) ou relayés
  // depuis une iframe enfant. On ne filtre plus sur event.source car un
  // message relayé depuis une iframe a un event.source différent de
  // `window` — on se fie uniquement au préfixe "PO_STRATEGY_".
  window.addEventListener("message", function (event) {
    const msg = event.data;
    if (!msg || typeof msg.source !== "string" || msg.source.indexOf("PO_STRATEGY_") !== 0) return;

    if (!isTop) {
      // Cette frame n'est pas la principale : on relaie vers le haut et
      // on ne fait aucun traitement local (pas d'overlay ici de toute façon).
      if (event.source === window) window.top.postMessage(msg, "*");
      return;
    }
  ...
  // Remonte depuis l'élément touché jusqu'au premier ancêtre qui ressemble
  // à un vrai bouton cliquable (évite de capturer un <span> décoratif imbriqué).
  function climbToActionable(el) {
    let node = el;
    for (let i = 0; i < 6 && node; i++) {
      ...
      if (/\bbtn\b|button/i.test(cls)) return node;
      node = node.parentElement;
    }
    return el; // rien trouvé : on garde l'élément touché tel quel
  }
  ...
  function onPickClick(e) {
    ...
    const el = climbToActionable(e.target);
